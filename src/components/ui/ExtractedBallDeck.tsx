...
export const ExtractedBallDeck: React.FC = () => {
  ...
  return (
    <div className="absolute top-6 left-1/2 transform -translate-x-1/2 z-20 flex flex-col items-center gap-3">
      {/* 방송 자막 바 */}
      <div className="bg-gradient-to-r from-sky-900/90 via-indigo-900/90 to-sky-900/90 backdrop-blur-md px-6 py-2 rounded-full border border-sky-400/40 shadow-xl text-sky-200 font-bold text-sm tracking-wide flex items-center gap-2">
        <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" />
        <span>{getStatusText()}</span>
      </div>

      {/* 2D HUD 당첨 덱 */}
      <div className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md px-6 py-3 rounded-2xl border border-slate-700/50 shadow-2xl">
        <div className="flex items-center gap-2.5">
          {Array.from({ length: 6 }).map((_, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-base text-slate-950 transition-all duration-500 shadow-lg ${
                  ball
